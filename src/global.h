...
Uint16StorageModel humidityDamperDisabledTimeParam(0x1205, &variableMutex, 0, 3600);     // [SECONDS] define tempo que o flap permanece desligado para controle em estágios
...
Uint16StorageModel alarmTemperatureTypeParam(0x1214, &variableMutex, 0, 3);              // define tipo de acionamento do alarme de temperatura (0=NENHUM, 1=BAIXO, 2=ALTO, 3=TODOS)
Uint16StorageModel alarmHumidityTypeParam(0x1215, &variableMutex, 0, 3);                 // define tipo de acionamento do alarme de humidade (0=NENHUM, 1=BAIXO, 2=ALTO, 3=TODOS)
Uint16StorageModel alarmSecurityTypeParam(0x1216, &variableMutex, 0, 3);                 // define tipo de acionamento do alarme do modo de segurança (0=NENHUM, 1=BAIXO, 2=ALTO, 3=TODOS)
BoolStorageModel alarmVentilationTypeParam(0x1217, &variableMutex);                      // define se gera alarme de ventilação (0=NÃO, 1=SIM)
BoolStorageModel alarmElectricalSupplyTypeParam(0x1218, &variableMutex);                 // define se gera alarme de falta de energia (0=NÃO, 1=SIM)
...
// VP 1300~1399
BoolStorageModel alarmEnabled(0x1300, &variableMutex);             // controlar se alarme pode ligar (0=NÃO, 1=SIM)
BoolStorageModel temperatureFanEnabled(0x1301, &variableMutex);    // controlar se a venoinha pode ligar (0=NÃO, 1=SIM)
BoolStorageModel injectionMachineEnabled(0x1302, &variableMutex);  // controlar se a injetora pode ligar (0=NÃO, 1=SIM)
...
// VP 1500~1599
Uint16StorageModel connectionStatus(0x1500, &variableMutex, 0, 3);             // estado da conexão wifi/servidor do dispositivo (0=DESCONECTADO, 1=CONECTANDO, 2=SEM_INTERNET, 3=CONECTADO)
Uint16StorageModel wifiSignalQuality(0x1501, &variableMutex, 0, 100);          // intensidade do sinal wifi do dispositivo
BoolStorageModel alarmOutputState(0x1502, &variableMutex);                     // indicador do estado da saida do alarme (0=DESLIGADO, 1=LIGADO)
Uint16StorageModel humidityDamperOutputState(0x1503, &variableMutex, 0, 2);    // indicador do estado da saida do damper de alarme (0=DESLIGADO, 1=ABRINDO, 2=FECHANDO)
BoolStorageModel temperatureFanOutputState(0x1504, &variableMutex);            // indicador do estado da saida da ventoinha (0=DESLIGADO, 1=LIGADO)
Uint16StorageModel injectionMachineOutputState(0x1505, &variableMutex, 0, 2);  // indicador do estado da saida da injetora (0=DESLIGADO, 1=LIGADO, 2=EM LIMPEZA)
